...
최근에 설계를 완전히 바꿔서 다시 한번 부탁드립니다.

제가 괜찮은 방향으로 나아가고있는지 확인받고 싶어 코드리뷰를 부탁드리게 되었습니다.

**질문입니다.**

유연한 코드를 작성하기 위해, 코드의 양을 줄여서 가독성을 높이기 위해 모듈화를 제대로 이해하고 싶습니다.

제가 작성한 코드가 유연성 있는 코드가 맞는지, 모듈화가 된 코드가 맞는지 궁급합니다!

※ 아래는 간략하게 정리한 영상입니다.

<https://youtu.be/OOWIdDPHG2o>

영상에 나오는 코드는 해당 깃허브 프로젝트에 업로드되어 있습니다.

설계는 대략적으로 다음과 같습니다.

[RPG Base System](#RPG-Base-System)

아래의 내용은 제가 생각하는 모듈화 방식과 제가 제대로했는지 헷갈리는 이유입니다.

**제가 생각하는 모듈화 방식**

1. 모듈화를 할때 상호참조 없이 위에서 아래로의 단방향 참조를 기준으로 잡았습니다. 예를 들면 캐릭터는 입력을 받아 움직여야 하지만 Controller가 Character를 참조하면 안 됩니다. 모듈화를 위한 설계상 둘 모두 GameManager의 아래에 있기 때문입니다. 때문에 Character를 이동시키기 위해서 Controller가 알아야 하는 정보는 GameManager가 책임지고 전달해 줍니다. Character를 그대로 넘기는게 아닌 인터페이스화 해서 넘깁니다.
2. Character가 자신의 애니메이션을 실행하기 위해 GetComponent를 사용해서 ITransitionAnimation들의 참조를 얻어 왔습니다. 결과적으로 Character는 자신이 어떤 애니메이션을 실행하는지 모릅니다. Combat과 MovementAnimation이 아닌  ITransitionAnimation를 가져왔기 때문입니다. 이 부분으로 인해 모듈화가 되었다고 생각합니다.

**헷갈리는 이유**

GameManager가 없으면 캐릭터는 입력을 받을 수 없습니다. GameManager가 Controller와 Character를 연결해주기 때문입니다. 그러나 모듈화가 되었으면 Character는 혼자서도 동작할 수 있어야 한다고 생각합니다. 하지만 Character 혼자만 있으면 입력을 받을 수 없기 때문에 이동과 점프, 공격, 패링등의 동작을할 수 없습니다. 

이에 대한 제 결론은 공중에서 떨어지거나 힘을 받으면 이동과 점프 애니메이션이 동작되니까 Character와 Controller에 대해 모듈화가 됐다 입니다. 또한 Combat과 패링은 캐릭터가 가져야 하는 기능이 아닌 GameManager가 있는 **플랫포머 게임** 내에서의 기능이기 때문에 모듈화가 올바로 됐다고 생각합니다.


**여기까지만 봐 주시면 될 것 같습니다. 감사합니다.**
...
# 문서 설명
이 문서에서 RPG(Role-playing game)는 플레이어블 캐릭터를 조작하여 몬스터와 전투하며 성장하는 역할 수행게임을 의미합니다.
