...
6. 게임은 스테이지 단위로 구성될 것이다.
7. 스테이지에서 스테이지로 넘어가는 방식은 씬로드일 것이다.
...
4. 스테이지는 순차적으로 넘어갈 것이다.
5. 모든 스테이지를 클리어하면 엔딩크래딧으로 넘어갈 것이다.
6. 엔딩크래딧이 끝나면 게임을 종료할 것이다.
7. 플레이중인 스테이지를 재시작할 수 있을 것이다.
8. 스테이지를 플레이중에 일시정지가 가능할 것이다.
9. 스테이지를 클리어하면 5초 뒤에 다음 스테이지로 넘어갈 것이다.
10. 캐릭터는 더블점프 혹은 그 이상을 못 할 것이다.
11. 패링된 오브젝트는 각자의 규칙에 맞게 움직일 것이다.
12. 패링은 동시에 여러 오브젝트에 적용될 수 있을 것이다.
13. 패링을 시도할 때 바람이 생성될 것이다.
14. 캐릭터의 이동속도는 변하지 않을 것이다.
15. 캐릭터는 점프 중 이동할 수 없을 것이다.
16. 캐릭터의 외형은 변할 수 있으나 종류는 하나일 것이다.
```
