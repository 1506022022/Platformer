...
# 문서 설명
이 문서에서 플랫포머 게임은 플레이어블 캐릭터를 조작하여 발판을 통해 목적지에 도달하는 것을 목표로 하며 [규칙](#이-게임의-규칙)을 만족하는 게임을 말합니다.

```
- 플랫포머 게임의 프레임워크에 대해 제가 어떠한 고민을 했는지 기록하고 되돌아보기 위해
...
7. 스테이지에 있는 모든 캐릭터가 시작위치에서 포탈까지 이동하면 스테이지가 클리어된다.
8. 캐릭터는 사망할 수 없다.
9. 싱글 플레이일 것이다.
```
**변할 수도 있는 규칙**
...
5. 스테이지에서 스테이지로 넘어가는 방식은 씬로드일 것이다.
