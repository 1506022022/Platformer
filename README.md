...
한 개체가 다양한 기능을 선택적으로 해야 할 경우 분기를 나누는 코드를 작성해서 기능을 구현하기 보다는 컴포넌트를 부착함으로써 구현되도록 하는 쪽이

코드량을 줄이고 결과적으로 코드의 가독성을 높일 수 있을 것 같다고 판단했기 때문에 이런 방식으로 구현했습니다. 
...
1. GetComponent 대신 GameManager에서 Combat을 생성해서 Character에 넣어주는식의 방식은 그만큼 GameManager의 역할이 늘어난다.
2. 컴포넌트를 부착하는 방식에 비해 Combat의 적용 여부를 옵션처럼 선택하기 어렵다.

**[업데이트 패턴]**

추가적으로 다음과 같은 문제들을 생각해 봤습니다.
1. MovementAnimation과 Combat의 업데이트 주기가 다른 경우, 더 잦은 주기에 맞춰서 업데이트를 호출해야 한다.
2. 형식에 맞춰 매개변수를 통일해야 한다.

주기의 문제는 입력을 처리하는 로직처럼 호출이 잦은 경우에는 오히려 더 효율적이고, 관리하기 좋은 코드가 만들어지기 때문에 경우에 따라 잘쓰면 괜찮다고 판단했습니다.
